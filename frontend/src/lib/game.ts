...
export class GameManager {
  ...
  async getGameStats() {
    ...
    return {
      ...
      currentRound: parseInt(currentRound?.toString() || '0'),
      status: (gameState as any)?.status || 'waiting',
      timeLeft: parseInt((gameState as any)?.timeLeft || '0'),
    };
  }
  ...
  async getRoundStats() {
    const currentQuestion = await redis.hgetall(RedisKeys.currentQuestion(this.roomId));
    if (!currentQuestion || !(currentQuestion as any).id) return null;
    ...
    for (const userEmail of survivors) {
      const answer = await redis.get(RedisKeys.userAnswer(userEmail, (currentQuestion as any).id.toString()));
      if (answer === 'A') {
        A_count++;
      } else if (answer === 'B') {
        B_count++;
    ...
    return {
      ...
      optionA: (currentQuestion as any).optionA,
      optionB: (currentQuestion as any).optionB,
      A_count,
      B_count,
      noAnswer_count,
